<div class="space-y-3">
    ...
    @if ($comingSoon)
        <flux:callout icon="exclamation-triangle" variant="warning" inline>
            ...
            <x-slot name="actions">
                <flux:button href="{{ admin_url('admin.php?page=wc-settings&tab=site-visibility') }}" target="_parent">
                    Manage visibility
                </flux:button>
            </x-slot>
        </flux:callout>
    ...
    @if ($myaccountPageId)
        ...
        @elseif(str_contains($myaccountContent ?? '', 'woocommerce_my_account'))
            <flux:callout icon="exclamation-triangle" variant="warning" inline>
                ...
                <flux:callout.text>Replace <code>[woocommerce_my_account]</code> with <code>[uvalab_my_account]</code>
                    in the My Account page.</flux:callout.text>
                <x-slot name="actions">
